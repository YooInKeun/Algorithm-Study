...
public class MovieDirector {
	...
	public static void main(String[] args) {
		...
		num = bf.nextInt();
		int time = 666; //제일 작은 값이 666이라서
		String title = "";
		
		while (true) {
			title = String.valueOf(time); // int형 time 변수를 string으로 변화시켜 title 에 넣는다
			// title 변수가 666을 포함하고 있으면
			if (title.contains(("666"))) 
				answer++;// answer 변수를 1키운다.
			// answer 과 입력값이 같으면
			if (answer == num) {
				answer = time; // time 값을 answer 에 넣는다.
				break;
			}
			time++;
		}
