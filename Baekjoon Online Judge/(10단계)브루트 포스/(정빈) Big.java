...
public class Big {
	static int[][] peopleArr; // 전체학생의 몸무게와 키를 담을 배열
	static int people; //전체학생수

	public static void main(String[] args) {
		Scanner bf = new Scanner(System.in);

		people = bf.nextInt(); //전체 학생수 받기
		peopleArr = new int[people][2]; 

		// 각 사람의 몸무게와 키를 담기
		for (int i = 0; i < people; i++) {
			peopleArr[i][0] = bf.nextInt();
			peopleArr[i][1] = bf.nextInt();
		}

		for (int i = 0; i < people; i++) {
			int answer = 0;//자신보다 키와 몸무게가 모두 큰사람
			//모든 학생과 자신의 키와 몸무게를 비교해서 크다면 answer 을 한명 늘려준다.
			for (int j = 0; j < people; j++) {
				if (peopleArr[j][0] > peopleArr[i][0] && peopleArr[j][1] > peopleArr[i][1])
					answer++;
			}
