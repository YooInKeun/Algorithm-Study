...
int main() {
	...
	int N;
	int weight[MAX], height[MAX], rank[MAX]; // weight: 몸무게, height: 신장, rank: 순위

	scanf("%d", &N);

	for (int i = 0; i < N; i++) // 순위 1로 초기화
		rank[i] = 1;

	for (int i = 0; i < N; i++) // 몸무게 및 신장 입력
		scanf("%d %d", &weight[i], &height[i]);

	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {

			if (weight[i] < weight[j] && height[i] < height[j]) // 몸무게와 신장, 두 수치 모두 큰 사람이 있으면 
				rank[i] += 1; // 순위 1 감소
		}
	}
