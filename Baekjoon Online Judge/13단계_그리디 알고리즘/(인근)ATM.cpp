...
int main() {
	...
	int N, people[MAX], peopleSum[MAX], sum = 0, answer = 0; // peopleSum: 각 사람이 (ATM기 사용을 기다리는 시간 + 돈을 인출하는 시간)
	...
	sort(people, people + N); // 오름차순으로 정렬해야, 최솟값을 구할 수 있다.
	...
	for (int i = 0; i < N; i++)
		answer += peopleSum[i]; // 각 사람이 걸리는 시간을 모두 더하기
