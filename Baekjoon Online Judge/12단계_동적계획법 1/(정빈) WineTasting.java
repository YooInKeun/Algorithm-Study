...
public class WineTasting {
	...
	static void drink(int wineNum, int time, int taste) {
		if (wineNum < wine.length-1) {
			if (time == 2) {
				drink(wineNum + 1, 0, taste);

			} 
			else {
				drink(wineNum + 2, time + 2, taste + wine[wineNum]+wine[wineNum+1]);
				drink(wineNum + 1, 0, taste);
			}
		} else {
			if (taste > answer) {
