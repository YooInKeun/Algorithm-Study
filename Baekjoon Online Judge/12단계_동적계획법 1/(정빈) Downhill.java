...
public class Downhill {
	...
	public static void search(int rowMap, int columMap) {
		if (rowMap == row - 1 && columMap == colum - 2) {
			if (map[rowMap][columMap] > map[row - 1][colum - 1]) {
				answer++;
				return;
			}

		} else if (rowMap == row - 2 && columMap == colum - 1) {
			if (map[rowMap][columMap] > map[row - 1][colum - 1]) {
				answer++;
				return;
			}
		...
		if (columMap == 0) {
			if (rowMap == row - 1) {
				// 오른쪽
				if (map[rowMap][columMap] > map[rowMap][columMap + 1]) {
					search(rowMap, columMap + 1);
				}
			} else {
				// 오른쪽
				if (map[rowMap][columMap] > map[rowMap][columMap + 1]) {
				...
				// 밑
				if (map[rowMap][columMap] > map[rowMap + 1][columMap]) {
					search(rowMap + 1, columMap);
				}
			}
		} else if (columMap == colum - 1) {
			// 왼쪽
			if (map[rowMap][columMap] > map[rowMap][columMap - 1]) {
				search(rowMap, columMap - 1);
			}
